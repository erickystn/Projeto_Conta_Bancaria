...
export class Input {
     ...
     *
     */
    static questionInt(pergunta: string, options?: BasicOptions): number {
        const readlinesync = require('readline-sync');
    ...
    static questionIntRange(pergunta: string, min: number, max: number): number {
        min = Math.floor(Math.abs(min));
        max = Math.floor(Math.abs(max));
        return Number(rls.question(pergunta, {
            limit: (input) => {
                const n = Number(input);
                // Verifica 3 coisas:
                // 1. É um número válido? (!isNaN)
                // 2. É inteiro? (Number.isInteger)
                // 3. Está entre 1 e 28?
                return !isNaN(n) && Number.isInteger(n) && n >= min && n <= max;
            },
            limitMessage: `Erro: O dia deve ser um número inteiro entre ${min} e ${max}.`
        }));
    }
